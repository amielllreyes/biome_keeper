...
type AuthStep = 'login' | 'forgotPassword' | 'verifySuccess';
...
export default function LoginPage() {
  ...
  // Check for email verification action in URL (still works for anyone
  // who previously received a real verification link)
  useEffect(() => {
    const handleEmailVerification = async () => {
      ...
      if (mode === 'verifyEmail' && actionCode) {
        try {
          ...
          setMessage({
            text: 'Invalid or expired verification link. Please sign in normally.',
            type: 'error',
          });
        } finally {
          setIsLoading(false);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!freshUser) throw new Error('User not found after reload');

      // No email verification check — all accounts (real or game usernames)
      // are allowed straight through. Security is handled by Firebase Auth,
      // Firestore rules, and role-based access control.
  ...
  const checkUserRoleAndRedirect = async (user: User) => {
    try {
      ...
      if (!userSnap.exists()) {
        // First web login — create Firestore profile
      ...
      if (error.code === 'failed-precondition') {
        ...
        setMessage({
          text: 'Unable to connect after multiple attempts. Please refresh the page.',
          type: 'error',
        });
      } else {
        setMessage({
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
        ...
        setMessage({ text: 'Failed to send reset email. Please try again.', type: 'error' });
      }
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="flex w-full md:w-1/2 justify-center items-center bg-white p-8">
        <AnimatePresence mode="wait">
          ...
          >
                ...
                {message.text && (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <button type="button" onClick={clearMessage} className="ml-2 text-current hover:opacity-70">
            ...
            {/* ── VERIFY SUCCESS (for anyone who clicks a real verification link) ── */}
            {step === 'verifySuccess' && (
            ...
            {step === 'forgotPassword' && (
              <form onSubmit={handleForgotPassword}>
                ...
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mb-6 text-gray-600">
